...
export default async (context, inject) => {
  ...
  let api = {}
  try {
    api = await Prismic.api('https://hotel-north-aust.prismic.io/api/v2', Object.assign({}, options,  {}))
  } catch (error) {
    console.error(error)
  ...
  let prismic = new Vue({
    computed: {
      ...
      apiEndpoint() {
        return 'https://hotel-north-aust.prismic.io/api/v2'
      },
      predicates() {
  ...
  if (process.client) {
    window.onNuxtReady && window.onNuxtReady((app) => {
      ...
      script.src = '//static.cdn.prismic.io/prismic.min.js?repo=hotel-north-aust&new=true'
